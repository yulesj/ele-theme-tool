...
// mix函数替换
exports.mixReplace = function (str) {
    const reg = /mix\((.*?)\)/g;
    let newStr = str.replace(reg, function (match) {
        if (match) {
            return match.replace('mix', 'mixToVar')
        }
        return match
    });
    return newStr
...
exports.varReplace = function (str, varConfig) {
    const reg = /var\((.*?)\)/g;
    let newStr = str.replace(reg, function (match) {
        if (match) {
            let key = match.replace('var(', '').replace(')', '')
            key = key.split(',')[0]
            const defaultColor = varToColor(key, varConfig)
            config.keyMap[key] = defaultColor;
            return `var(${key},${defaultColor})`
        }
        return match
    });
    return newStr
